...
export const GridEnvSchema = z
  ...
  .refine(
    data => {
      return (
        data.policy.length === data.rows &&
        data.policy.every(row => row.length === data.cols)
      );
    },
    {
...
export function createDefaultGridEnv(): GridEnv {
  return {
    ...
    policy: Array.from({ length: 5 }, () =>
      Array.from({ length: 5 }, () => "stay" as const)
    ),
  };
}
...
export function getRewardTensor(env: GridEnv) {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      rewardMatrix.push(
        getActionResult(env, r, c, safeGetCellPolicy(env, r, c)).reward
      );
    }
  }
